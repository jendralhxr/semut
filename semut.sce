...
while unvisited > 0 do
    for id= 1:vehicle_count do
        ...
        // back to depot if not carrying enough payload for next pos' demand
...
// mu = visibility, re-initialization
for i= 1:26 do
    for j= 1:26 do
        if i==j then 
            mu(i,j)= 0;
...
// let's deploy some pile of ants :D
for generation= 1:generation_max do
    ...
    while unvisited > 0 do
        for id= 1:vehicle_count do
            pos_next=0;
            [foo,pos_next]= max(tau_temp(pos_current(id),:));
    ...
    // insertion move goes here!!
    route_temp= route_aco;
    for id= 1:vehicle_count do
        for i= 2:step(id)-1 do
            for j= 2:step(id)-1 do
                // swap test
                if distance(route_aco(id,i),route_aco(id,j+1)) +distance(route_aco(id,i),route_aco(id,j-1)) + distance(route_aco(id,j),route_aco(id,i+1)) +distance(route_aco(id,j),route_aco(id,i-1)) < distance(route_aco(id,i),route_aco(id,i+1)) +distance(route_aco(id,i),route_aco(id,i-1)) + distance(route_aco(id,j),route_aco(id,j+1)) +distance(route_aco(id,j),route_aco(id,j-1)) then
                    temp= route_temp(id,i);
                    route_temp(id,i)= route_temp(id,j);
                    route_temp(id,j)= temp;
                end
                eligible= 1;
                for n= 1:step(id) do
                    if route_temp(id,n)==1 then 
                        payload(id)=payload_max;
                    end
                    payload(id)= payload(id)-demand(route_temp(id,n))
                    // ineligilbe route due to payload insufficiency
                ...
                if eligible==1 then
                    trail(route_aco(id,i),route_aco(id,i-1))= 0;
                    trail(route_aco(id,i),route_aco(id,i+1))= 0;
                    trail(route_aco(id,j),route_aco(id,j-1))= 0;
                    trail(route_aco(id,j),route_aco(id,j+1))= 0;
                    trail(route_aco(id,i),route_aco(id,j-1))= 1;
                    trail(route_aco(id,i),route_aco(id,j+1))= 1;
                    trail(route_aco(id,j),route_aco(id,i-1))= 1;
                    trail(route_aco(id,j),route_aco(id,i+1))= 1;
                    route_aco= route_temp;
                    //mprintf("swap!!\n");
                end
            end
        end
    ...
    for id= 1:vehicle_count do
        ...
        cost(id)= distance_total(id)/fuel_ratio*fuel_price;
        mprintf('aco gen=%d id=%d dist=%.2f cost=%.2f\n',generation,id,distance_total(id),cost(id));
    end
    distance_aco= sum(distance_total);

    // global route update in case the newest is shorter
    if distance_aco<distance_gb then
        distance_gb= distance_aco;
        route_gb= route_aco;
        trail_gb= trail;
    end
    ...
    for i= 1:26 do
        for j= 1:26 do
            ...
            // ini trail dari l_gb
            if trail_gb(i,j)==1 then 
                tau(i,j)= tau(i,j)+c_alpha*(1/distance_gb);
            end
        end
    end    

end
// ini yang baru--generasi

mprintf('Kesimpulan: jarak %.2f km, biaya Rp %.0f\n',distance_gb,distance_gb/fuel_ratio*fuel_price);

// for author's own good, don't mind these two lines
route_gb(:,:)=route_aco(:,:)-1;
route_nn(:,:)=route_nn(:,:)-1;
